...
def start_button(client):
    ...
    if FORCE_SUB_CHANNEL and not FORCE_SUB_GROUP and not FORCE_SUB_GROUP2:
        buttons = [
            [
                InlineKeyboardButton(text="GRUOP", url=client.invitelink),
            ],
            [
                InlineKeyboardButton(text="• ᴛᴇɴᴛᴀɴɢ sᴀʏᴀ •", callback_data="about"),
                InlineKeyboardButton(text="• ᴛᴜᴛᴜᴘ •", callback_data="close"),
...
def fsub_button(client, message):
    if not FORCE_SUB_CHANNEL and FORCE_SUB_GROUP and not FORCE_SUB_GROUP2:
        buttons = [
            [
                InlineKeyboardButton(text="𝗝𝗼𝗶𝗻", url=client.invitelink2),
            ],
        ]
        try:
            buttons.append(
    ...
    if FORCE_SUB_CHANNEL and not FORCE_SUB_GROUP and not FORCE_SUB_GROUP2:
        buttons = [
            [
                InlineKeyboardButton(text="𝗝𝗼𝗶𝗻", url=client.invitelink),
            ],
        ]
        try:
            buttons.append(
    ...
    if FORCE_SUB_CHANNEL and FORCE_SUB_GROUP and FORCE_SUB_GROUP2:
        buttons = [
            ...
            [
                InlineKeyboardButton(text="𝗝𝗼𝗶𝗻", url=client.invitelink3),
            ],
        ]
        try:
            buttons.append(
